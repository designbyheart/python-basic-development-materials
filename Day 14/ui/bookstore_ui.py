...
class BookstoreUI:
    ...
    def create_widgets(self):
        ...
        self.details_button = tk.Button(
            self.root, text="Show Details", command=self.show_book_details
        )
        self.details_button.pack(pady=10)

        self.clear_selection_button = tk.Button(
            self.root, text="Clear Selection", command=self.clear_book_selection
        )
        self.clear_selection_button.pack(pady=10)

    def clear_book_selection(self):
    ...
    def show_book_details(self):
        try:
            ...
            messagebox.showwarning(
                "Selection Error", "Please select a book from the list."
            )
